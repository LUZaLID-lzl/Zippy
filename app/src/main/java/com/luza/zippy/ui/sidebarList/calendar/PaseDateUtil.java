...
class PaseDateUtil {
    ...
    // 1900-2100年的节气C值
    private static final double[] CENTURY_VALUE = {
        3.87, 18.73, 5.63, 20.646, 4.81, 20.1, 5.52, 21.04, 5.678, 21.37, 
        7.108, 22.83, 7.5, 23.13, 7.646, 23.042, 6.538, 21.94, 5.478, 20.12, 
        4.438, 18.73, 3.37, 17.73
    };

    // 节气偏移值
    private static final double[] TERM_DEVIATION = {
        0.004, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002,
        0.006, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002, 0.002,
        0.002, 0.002, 0.002, 0.002
    };

    // 节气基本日期
    private static final double[] TERM_BASE_DATE = {
        5.4055, 20.12, 3.87, 18.73, 5.63, 20.646, 4.81, 20.1, 5.52, 21.04,
        5.678, 21.37, 7.108, 22.83, 7.5, 23.13, 7.646, 23.042, 6.538, 21.94,
        5.478, 20.12, 4.438, 18.73
    };
    ...
    public static String getSolarTerm(int year, int month, int day) {
        ...
        // 计算世纪数
        double century = (year - 1900) / 100.0;
        
        // 计算月份对应的节气索引
        int termIndex = (month - 1) * 2;
        
        // 计算第一个节气日期
        double term1 = TERM_BASE_DATE[termIndex] + 
            (century * TERM_DEVIATION[termIndex]) +
            CENTURY_VALUE[termIndex] * century * century;
        
        // 计算第二个节气日期
        double term2 = TERM_BASE_DATE[termIndex + 1] + 
            (century * TERM_DEVIATION[termIndex + 1]) +
            CENTURY_VALUE[termIndex + 1] * century * century;

        // 修正日期误差
        term1 = Math.floor(term1 + 0.5);
        term2 = Math.floor(term2 + 0.5);

        // 判断是否是节气日
        if (day == (int)term1) {
            return SOLAR_TERMS[termIndex];
        } else if (day == (int)term2) {
